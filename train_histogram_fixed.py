...
if __name__ == "__main__":
    ...
    train_data   = np.load(OUTPUT_DIR / "train_data.npy")
    train_labels = np.load(OUTPUT_DIR / "train_labels.npy")
    val_data     = np.load(OUTPUT_DIR / "val_data.npy")
    val_labels   = np.load(OUTPUT_DIR / "val_labels.npy")

    print(f"Train: {train_data.shape}  Val: {val_data.shape}")

    train_ds = FixedWindowDataset(train_data, train_labels, augment=True)
    val_ds   = FixedWindowDataset(val_data,   val_labels,   augment=False)
    ...
    print("=" * 50)
    print(f"Training — fixed {WINDOW_MS}ms windows")
    print("=" * 50 + "\n")
    ...
    metrics_path = OUTPUT_DIR / 'histogram_fixed_metrics.txt'
