...
def merge():
    print("=" * 60)
    print("MERGE TIMESURFACE: batches -> final dataset")
    print("=" * 60)
    ...
    out_data   = SLIDING_DIR_T7 / "timesurface_data.npy"
    out_labels = SLIDING_DIR_T7 / "timesurface_labels.npy"
    out_recids = SLIDING_DIR_T7 / "timesurface_recording_ids.npy"
    ...
    for i, (df, lf, rf) in enumerate(zip(data_files, label_files, recid_files)):
        ...
        print(f"done  ({current_idx:,}/{total_samples:,})")
    ...
    print("\nMERGE COMPLETE")
    print(f"Output: {SLIDING_DIR_T7}")
    print("Next step: run train_timesurface.py")
