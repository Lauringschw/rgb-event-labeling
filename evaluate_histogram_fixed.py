...
TEST_DIR    = OUTPUT_DIR / "test_samples"
...
def evaluate_rq1(model, device):
    ...
    results = {}
    lines   = ["RQ1 — Window Length Effect (Histogram Fixed)"]
...
def evaluate_rq2(model, device):
    ...
    results = {}
    lines   = ["RQ2 — Temporal Landmark Effect (Histogram Fixed)"]
...
def evaluate_rq3(rq1_results):
    print("\n" + "=" * 55)
    print("RQ3 — Representation Comparison (Histogram Fixed)")
    print("=" * 55)
    acc_30ms = rq1_results[30]
    print(f"Histogram accuracy at τ=0, Δt=30ms: {acc_30ms:.2f}%")
    np.save(RESULTS_DIR / "rq3_histogram_fixed_acc_30ms.npy", np.array([acc_30ms]))
    (RESULTS_DIR / "rq3_result.txt").write_text(
        f"RQ3 — Histogram Fixed at τ=0, Δt=30ms\n{acc_30ms:.4f}\n")
    print(f"Saved -> {RESULTS_DIR}/rq3_*")
...
if __name__ == "__main__":
    print("=" * 55)
    print("EVALUATION — Histogram Fixed CNN (RQ1 + RQ2 + RQ3)")
    print("=" * 55)

    device = get_device()
    print(f"\nDevice: {device}")

    model_path = OUTPUT_DIR / "model_histogram_fixed_best.pth"
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model not found: {model_path}\nRun train_histogram_fixed.py first.")
    ...
    for fname in ["rq1_data.npy", "rq1_labels.npy", "rq1_durations_ms.npy",
        ...
        if not (TEST_DIR / fname).exists():
            raise FileNotFoundError(
                ...
                f"Run extract_test_samples_histogram_fixed.py first.")
