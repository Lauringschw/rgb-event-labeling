...
def evaluate_rq1(model, device):
    ...
    results = {}
    lines   = ["RQ1 — Window Length Effect (Voxel Grid)"]
...
def evaluate_rq2(model, device):
    ...
    results = {}
    lines   = ["RQ2 — Temporal Landmark Effect (Voxel Grid)"]
...
def evaluate_rq3(rq1_results):
    print("\n" + "=" * 55)
    print("RQ3 — Representation Comparison (Voxel contribution)")
    print("=" * 55)

    acc_30ms = rq1_results[30]
    print(f"Voxel grid accuracy at τ=0, Δt=30ms: {acc_30ms:.2f}%")

    np.save(RESULTS_DIR / "rq3_voxel_acc_30ms.npy", np.array([acc_30ms]))
    (RESULTS_DIR / "rq3_voxel_result.txt").write_text(
        f"RQ3 — Voxel grid accuracy at τ=0, Δt=30ms\n{acc_30ms:.4f}\n")
    print(f"Saved -> {RESULTS_DIR}/rq3_voxel_*")
...
if __name__ == "__main__":
    print("=" * 55)
    print("EVALUATION — Voxel CNN (RQ1 + RQ2 + RQ3)")
    print("=" * 55)
    ...
    if not model_path.exists():
        raise FileNotFoundError(f"Model not found: {model_path}\nRun train_voxel.py first.")

    model = VoxelCNN().to(device)
    ...
    for fname in ["rq1_data.npy", "rq1_labels.npy", "rq1_durations_ms.npy",
        ...
        if not (TEST_DIR / fname).exists():
            raise FileNotFoundError(
                ...
                f"Run extract_test_samples_voxel.py first.")
